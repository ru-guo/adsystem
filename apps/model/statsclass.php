...
class Model_StatsClass
{
		...
		public function adsplanstatussqlsnums( )
		{
				...
				{
						$sql = "SELECT s.*  ".$where3.( " FROM zyads_planstats as s , zyads_plan as p WHERE  1 and s.planid=p.planid ".$filesize."  {$where1} {$where2}  ORDER BY id DESC" );
		...
		public function tplanstatsqlsandnum( )
		{
				...
				else
				{
						if ( $actiontype == "dostats" )
						{
								$sql = "SELECT s.* FROM zyads_planstats as s , zyads_plan as p WHERE  1 and s.planid=p.planid and s.uid=".$_SESSION['uid']." ".$dwhere.( " ".$filesize."  ORDER BY s.id DESC" );
								$array = $this->dbo->get_all( $sql );
								return $array;
						}
						if ( $planid )
		...
		public function getuserdata( $plantype, $usertype )
		{
				...
				$sql = "SELECT ".$psum.( " As n,sum(s.views) As v ,".$sumsumpay." AS yf FROM {$crrentpage} AS s ,zyads_plan AS p,  zyads_users AS u WHERE day='" ).DAYS."' AND s.planid=p.planid AND s.uid=u.uid AND p.plantype='".$plantype.( "' ".$prexuid." {$groupby}" );
				if ( $usertype == "aff" )
				{
						$b = $this->dbo->get_one( $sql );
						return $b;
		...
		public function sumpaystatsplans( $day, $usertype )
		{
				...
				if ( $usertype == "adv" )
				{
						$sql = "SELECT sum(s.sumadvpay) AS yf,p.planid FROM zyads_planstats AS s ,zyads_plan AS p WHERE s.day='".$day."' AND s.planid=p.planid  AND p.uid=".$_SESSION['uid']." GROUP BY p.planid ";
